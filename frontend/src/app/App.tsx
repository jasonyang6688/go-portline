...
export default function App() {
  ...
  function applyMonitorSnapshot(snapshot: MonitorSnapshot) {
    setMonitorSnapshot(snapshot);
    setMonitorHistory((current) => appendMonitorHistory(current, snapshot));
  }

  async function refreshPersistedMonitorHistory(sessionId: string, connectionId: string) {
    if (!backendAvailable) {
  ...
  function handleMonitorRulesChange(rules: MonitorRules) {
    setMonitorRules(rules);
    saveMonitorRules(rules);
  }

  async function copyMonitorIncidentReport(): Promise<string> {
    if (!activeSession) {
